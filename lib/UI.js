...
var easyExtender  = require("easy-extender");
 ...
 * @returns {UI}
 */
var UI = function (bs, opts, emitter) {
     ...
     *
     */
    ui.pluginManager = new easyExtender(defaultPlugins, hooks).init();
...
UI.prototype.init = function () {
     ...
     * @type {*}
     */
    var plugins = ui.bs.options.get('plugins');

    ui.options   = ui.options.set('plugins', plugins);
    ui.bsPlugins = require("./resolve-plugins")(plugins);
...
function tasksComplete (ui) {
    ...
    return function (err) {
        ...
        ui.bs.setOption('urls', function (urls) {
            return urls.mergeDeep(ui.options.get('urls'));
        }).subscribe(function () {
            /**
             * Running event
             */
            ...
            /**
             * Finally call the user-provided callback to signal completion
             */
            ui.cb(null, ui);
        })

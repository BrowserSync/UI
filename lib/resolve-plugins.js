...
module.exports = function (plugins) {
    return require("immutable")
        ...
        .map(function (plugin) {
            ...
            var templates = plugin.getIn(["pkg", "browser-sync:ui", "hooks", "templates"]);
            var markup    = plugin.getIn(["pkg", "browser-sync:ui", "hooks", "markup"]);
            var templateObj;

            if (templates.size) {

                templateObj = resolvePluginFiles(templates, plugin.get('relpath'));

                if (templateObj.size) {
                    plugin = plugin.set("templates", templateObj);
                }
        ...
        .map(function (plugin) {
            ...
            return resolveIfPluginHas(["pkg", "browser-sync:ui", "hooks", "client:js"], plugin);
        });
};
 ...
 * @returns {*}
 */
function resolveIfPluginHas(optPath, plugin) {

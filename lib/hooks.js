...
module.exports = {
    ...
    "client:js": function (hooks, ui) {
         ...
         */
        ui.options.get('plugins').forEach(function (plugin) {
    ...
    "templates": function (hooks, initial, ui) {
         ...
         * @type {string}
         */
        var pluginDirectives = ui.options.get('plugins').reduce(function (all, plugin) {
...
function preAngular (plugins, config, ui) {
    ...
    return Object.keys(plugins)
        ...
        .map(function (key) {
            if (key === "plugins") {
                var pluginMarkup = ui.options.get('plugins')
                    .filter(function (item) {
                        return item.hasIn(['module', 'browser-sync:ui']);
                    })
                    .reduce(function (all, item, i) {
                    all += pluginItemTmpl
                        .replace("%content%", item.get("markup") || "")
                        .replace(/%index%/g, i)
                        .replace(/%name%/g, item.getIn(["module", "plugin:name"]));

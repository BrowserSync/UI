(function (angular) {
    ...
    function SyncOptionsController(Socket, options, pagesConfig) {
        ...
        var defaults = ctrl.options.clientOptions['default'];

        ctrl.syncItems.push(addItem("codeSync", ["codeSync"], defaults.codeSync, taglines["codeSync"]));

        Object.keys(defaults.ghostMode).forEach(function (item) {
            if (item !== "forms" && item !== "location") {
                ctrl.syncItems.push(addItem(item, ["ghostMode", item], defaults.ghostMode[item], taglines[item]));
            }
        });

        Object.keys(defaults.ghostMode.forms).forEach(function (item) {
            ctrl.syncItems.push(addItem("Forms: " + item, ["ghostMode", "forms", item], defaults.ghostMode["forms"][item], taglines["ghostMode." + item]));
        });

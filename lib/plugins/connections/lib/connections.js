...
module.exports.init = function (ui, bs) {
    ...
    function clientFilter (incoming) {
        return incoming.filter(function (item, key) {
            return key !== 'default';
        }).toList().toJS();
    }
    ...
    //        var filtered = incoming.filter(function (item, key) {
    //            return key !== 'default';
    //        }).toList().toJS();
    //        //cleints = incoming;
    ...
    Rx.Observable
        ...
        .combineLatest(bs.clients$, function (_, clients) {

...
module.exports.init = function (ui, bs) {
    ...
    ui.listen('connections', {
        highlight: function (connection) {
            var match = bs.clients$.getValue().filter(x => x.get('id') === connection.id).toList();
            if (match.size) {
                match = match.toJS()[0];
                ui.clients.sockets.forEach(function (cl) {
                	if (cl.id === match.socketId) {
                        cl.emit('highlight')
                    }
                })
            }
    ...
    //    .withLatestFrom(bs.clients$, function (uiClient, clients) {
    //        return {uiClient: uiClient, clients: clients};
    //    })
    //    .subscribe(function (x) {
    //        console.log('connection', x.clients.size);
    //        x.uiClient.emit('ui:connections:update', x.clients.toList().toJS());
    //    });

...
module.exports.init = function (ui, bs) {
    ...
    var methods = {
        ...
        sendUpdatedUrls: function (urls) {
            ui.socket.emit("cp:history:update", decorateUrls(urls));
        },
        /**
         ...
         */
        getVisited: function () {
            ui.socket.emit("cp:receive:visited", decorateUrls(validUrls));
        }
    };

    ui.clients.on("connection", function (client) {
        client.on("cp:history:connected", methods.addPath);
    });

    ui.socket.on("connection", function (cpClient) {
        /**

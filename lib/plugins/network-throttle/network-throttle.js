    ...
    var methods = {
        ...
        "server:create": function (data) {
            ...
            function createThrottle (err, port) {
                ...
                if (ui.bs.options.get("scheme") === "https") {
                    var cert  = ui.bs.config.certs;
                    args.key  = fs.readFileSync(cert.key);
                    args.cert = fs.readFileSync(cert.cert);
                }
             ...
             * Try for a free port
             */
            portscanner.findAPortNotInUse(data.port, data.port + 100, "127.0.0.1", function (err, port) {
                if (err) {
